...
GFX_Modifier gfx_get_modifiers(void) {
  GFX_Modifier modifier_mask = 0;

  if(GetKeyState(VK_CONTROL) & 0x8000) {
    modifier_mask |= GFX_MOD_CONTROL;
  }

  if(GetKeyState(VK_SHIFT) & 0x8000) {
    modifier_mask |= GFX_MOD_SHIFT;
  }

  if(GetKeyState(VK_MENU) & 0x8000) {
    modifier_mask |= GFX_MOD_ALT;
  }

  if((GetKeyState(VK_LWIN) & 0x8000) || (GetKeyState(VK_RWIN) & 0x8000)) {
    modifier_mask |= GFX_MOD_META;
  }
...
GFX_Event* gfx_win32_event_push(Arena *a, GFX_EventList *event_list, GFX_EventKind event_kind) {
  GFX_Event *event = push_struct_no_zero(a, GFX_Event);
  event->kind = event_kind;
  event->modifier_mask = gfx_get_modifiers();
...
GFX_Event* gfx_win32_event_pop(GFX_EventList *event_list) {
  GFX_Event *event = event_list->last;
...
GFX_Key gfx_win32_key_from_virtual_keycode(WPARAM virtual_keycode) {
  ...
  switch(virtual_keycode) {
    ...
    key = GFX_KEY_LEFT_META;
    break;
    case VK_RWIN /* 0x5C	Right Windows logo key */:
    key = GFX_KEY_RIGHT_META;
    break;
    // 0x5E	Reserved
